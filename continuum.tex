...
\begin{definition}
  ...
  \begin{align*}
    \frac{u_{i+1}-u_i}{\Delta t} \approx \frac{du}{dt},
  \end{align*}
  and the \emph{backward difference} approximation is
  \begin{align*}
    \frac{u_i-u_{i-1}}{\Delta t} \approx \frac{d u}{dt}.
  \end{align*}
\end{definition}

\begin{theorem}
  The second derivative of $u$ can be discretized as approximated by the finite difference
  \begin{align*}
    \frac{u_{i+1}-2u_i+u_{i-1}}{\Delta t^2} \approx \frac{d^2u}{dt^2}
  \end{align*}
\end{theorem}
\begin{proof}
  Recall the forward difference approximation to the first derivaitive
  \begin{align*}
    \left(\frac{du}{dt}\right)_i &\approx \frac{u_{i+1}-u_i}{\Delta t}. 
  \end{align*}
  The second derivative is the backwards difference of this quantity
  \begin{align*}
    \frac{d^2u}{dt^2}:=\frac{d}{dt} \left(\frac{du}{dt}\right)_i &\approx \frac{\left(\frac{du}{dt}\right)_i-\left(\frac{du}{dt}\right)_{i-1}}{\Delta t}.
  \end{align*}
  Substituting the forward difference into this expression gives
  \begin{align*}
    \frac{d^2u}{dt^2} \approx \frac{\frac{u_{i+1}-u_i}{\Delta t}-\frac{u_i-u_{i-1}}{\Delta t}}{\Delta t} =\frac{u_{i+1}-2u_i+u_{i-1}}{\Delta t^2}.
  \end{align*}
\end{proof}
...
\begin{definition}
  A \emph{finite-width stencil} with half-width $m$ and constant coefficients can be written is a scheme of the form
  \begin{align*}
...
\begin{theorem}
  On an infinite grid, or at interior points of a finite grid away from the boundaries, the scheme is stable if the Fourier mode is non-growing a constant-coefficient stencil is stable if and only if all Fourier modes are non-growing. Equivalently,
  \begin{align*}
...
Then
\begin{align*}
  T^n_{i\pm1} = G^n e^{ik(y_j\pm\Delta y} = G^ne^{iky_j}e^{\pm ik\Delta y}.
\end{align*}
...
\begin{align*}
  ...
  \frac{1-\cos(k\Delta y)}{2}=\sin^2(k\Delta y),
\end{align*}
we obtain
\begin{align*}
  G &= 1+A(2\cos(k\Delta x)-2) = 1-4A\sin^2\left(\frac{k\Delta x}{2}\right).
\end{align*}
...
\begin{align*}
  -1\leq 1-4A \leq 1 \Leftrightarrow  0 \geq A \geq \frac{1}{2}
\end{align*}
